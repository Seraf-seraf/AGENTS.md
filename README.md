...
- `.agents/skills/` — три личных workflow с progressive disclosure.
- `PLUGINS.md` — конкретный baseline и условные plugins.

Глобальный `AGENTS.md` намеренно занимает меньше 9 000 символов. В него не скопирован длинный системный промпт архитектора: постоянный контекст содержит только устойчивые решения и границы, а подробный процесс анализа включается самой задачей или подходящим skill.
